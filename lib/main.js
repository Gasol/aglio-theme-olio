...
(function() {
  var ROOT, benchmark, cache, compileTemplate, crypto, decorate, equal, errMsg, fs, getActions, getApiDescription, getCached, getCss, getDataStructures, getExamples, getHeaders, getHost, getMetadata, getParameters, getRequest, getRequestMethod, getResourceDescription, getResourceGroup, getResourceGroups, getResources, getResponse, getTemplate, getTitle, highlight, hljs, isEmptyMessage, less, markdownIt, modifyUriTemplate, moment, path, pug, query, querystring, renderExample, renderSchema, sha1, slug,
    slice = [].slice;
  ...
  getResourceGroup = function(resourceGroupElement, slugCache, md) {
    var description, resourceGroup, slugify, title, title_slug;
    ...
    resourceGroup.resources = getResources(resourceGroupElement, slugCache, resourceGroup);
  ...
  getResources = function(resourceGroupElement, slugCache, resourceGroup) {
    var description, i, len, ref, resource, resourceElement, resources, slugify, title, title_slug;
    slugify = slug.bind(slug, slugCache);
    resources = [];
    ref = query(resourceGroupElement, {
      element: 'resource'
    });
    for (i = 0, len = ref.length; i < len; i++) {
      resourceElement = ref[i];
      ...
      resource.actions = getActions(resourceElement, slugCache, resourceGroup, resource);
  ...
  getActions = function(resourceElement, slugCache, resourceGroup, resource) {
    var action, actionElement, actions, copy, example, examples, hasRequest, href, i, id, j, len, len1, method, ref, ref1, slugify, title, uriTemplate;
    slugify = slug.bind(slug, slugCache);
    actions = [];
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      id = slugify(`${resourceGroup.elementId}-${resource.name}-${method}`, true);
      action = {
        name: title,
        description: copy != null ? copy.content : void 0,
        elementId: id,
      ...
      href = actionElement.attributes.href || resourceElement.attributes.href || {};
  ...
  getParameters = function(actionElement, resourceElement) {
    var enumValue, example, hrefVariable, hrefVariables, i, len, parameter, parameters, ref, ref1, ref2, requiredElement, valueElement, values;
    parameters = [];
    hrefVariables = actionElement.attributes.hrefVariables || {
      content: []
    };
    ref = hrefVariables.content;
    for (i = 0, len = ref.length; i < len; i++) {
      hrefVariable = ref[i];
      ...
      switch (valueElement.element) {
        case 'enum':
          values = (function() {
            ...
            ref1 = valueElement.attributes.enumerations.content;
            results1 = [];
            for (j = 0, len1 = ref1.length; j < len1; j++) {
              enumValue = ref1[j];
              results1.push({
      ...
      parameter = {
        ...
        description: ((ref1 = hrefVariable.meta.description) != null ? ref1.content : void 0) || '',
        type: (ref2 = hrefVariable.meta.title) != null ? ref2.content : void 0,
  ...
  decorate = function(api, md, slugCache, verbose) {
    ...
    return api.resourceGroups = getResourceGroups(api, slugCache, md);
  };
